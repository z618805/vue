				...
				<el-col :span="24" class="pin background-color-bai">
					<el-col :span="24" class="pin">
						...
						<el-col :span="20" class="cor xiao-size">
							<div v-for="(list,index) in arr" :key="index" :span="2" class="cor xiao-size cursor pin3 center floj" ><span class="xiao-size" :class='{ellan1:index===isactive}' @click="dianji(index)">{{list.name}}</span></div>
						</el-col>
					</el-col>
					<el-col :span="24" class="pin">
						<el-col :span="1" class="cor xiao-size">版本：</el-col>
						<div v-for="(list,index) in arr2" :key="index" class="cor xiao-size cursor center floj pin3" ><span class="xiao-size" :class='{ellan1:index===isactive2}' @click="dianji2(index)">{{list.verName}}</span></div>
					</el-col>
					<el-col :span="24" class="pin">
						<el-col :span="1" class="cor xiao-size">年级：</el-col>
						<el-col :span="20">
							<div v-for="(list,index) in arr1" :key="index" :span="2" class="cor xiao-size cursor pin3 center floj" ><span class="xiao-size" :class='{ellan1:index===isactive1}' @click="dianji1(index)">{{list.volumeName}}</span></div>
						</el-col>
					</el-col>
				</el-col>
			</el-col>
			...
			<el-col :span="16" style="margin-top: .5rem;">
				...
				<el-col :span="20" class="pin1">
					<el-col :span="24" class="background-color-bai height">
						<el-col :span="24" class="pin">
							<div :class="{xuanfsa:quan==leixing}" style="padding: .3rem .5rem;" class="floj1 cursor pin center bodekd" @click="xuanzes(-1)">全部</div>
							<div v-for="(list,index) in lei" style="padding: .3rem .5rem;" :class="{xuanfsa:index==leixing}" class="floj1 cursor pin center bodekd" @click="xuanzes(index)">{{list.restypeName}}</div>
						</el-col>
